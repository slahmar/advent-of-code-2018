...
with open('15.txt', 'r') as file:
    ...
    cavern = [list(line) for line in lines]
    #print('\n'.join([''.join(line) for line in cavern]))
        ...
        for unit in sorted(units, key=lambda unit: (unit.x, unit.y)):
            if unit.alive:
                ...
                if len(other_units) == 0:
                    if path is None:
                        #print(f'No target could be found for {unit}')
                        target_found = False
                        break
    ...
    print(f'Number of full rounds {i}, sum of hit points {sum_of_hit_points}')
    print(f'Answer {i*sum_of_hit_points}')
